...
def test_r1_1_user_register():
    ...
    assert register('u0', 'user0@test.com', 'Test123_') is 0
    assert register('u1', 'user1@test.com', 'Test123_') is 0
    # empty name
    assert register('', 'test2@test.com', 'Test123_') is not 0
    # empty email
    assert register('u2', '', 'Test123_') is not 0
...
def test_r1_3_user_register():
    ...
    assert register('u0', 'test1@testcom', 'Test123_') is not 0
    # illegal email (multiple @)
    assert register('u0', 'test1@@test.com', 'Test123_') is not 0
    # illegal email (missing domain after @)
    assert register('u0', 'test1@.com', 'Test123_') is not 0
...
def test_r1_4_user_register():
    ...
    """
    # Testing password
    assert register('u0', 'p0@test.com', 'Test123_') is 0
    # empty password (falls under password too short)
    assert register('u0', 'p1@test.com', '') is not 0
    # missing special character
    assert register('u0', 'p1@test.com', 'Test123') is not 0
    # missing number
    assert register('u0', 'p1@test.com', 'Test_') is not 0
    # missing number and special character
    assert register('u0', 'p1@test.com', 'Test') is not 0
    # does not have upper case letter
    assert register('u0', 'p1@test.com', 'test_dsak123') is not 0
    # does not have lower case letter
    assert register('u0', 'p1@test.com', 'TEST123_') is not 0
    # one that works to prove username and email is valid
    assert register('u0', 'p1@test.com', 'TESTpassword_123*^%') is 0
    # illegal email (missing content before @)
    assert register('u0', '@test.com', 'Test123_') is not 0
    # one that works to prove username and password is valid
    assert register('u0', 'test3@test.com', 'Test123_') is 0
...
def test_r1_5_user_register():
    ...
    """
    # illegal character
    assert register('u0.', 'test2@test.com', 'Test123_') is not 0
    # space as a prefix
    assert register(' u0', 'test2@test.com', 'Test123_') is not 0
    # space as a suffix
    assert register('u0 ', 'test2@test.com', 'Test123_') is not 0
...
def test_r1_6_user_register():
    ...
    """
    # name too short
    assert register('X', 'test2@test.com', 'Test123_') is not 0
    # name too long
    assert register('this is a very long name that the '
                    'program cannot possibly handle',
                    'test2@test.com', 'Test123_') is not 0
    # different variations of allowed names
    assert register('u 0', 'user2@test.com', 'Test123_') is 0
    assert register('u   0', 'user3@test.com', 'Test123_') is 0
    assert register('TEST 0', 'user4@test.com', 'Test123_') is 0
    assert register('Test', 'user5@test.com', 'Test123_') is 0
...
def test_r1_7_user_register():
    ...
    assert register('u0', 'test0@test.com', 'Test123_') is 0
    assert register('u0', 'test1@test.com', 'Test123_') is 0
    # repeated email
    assert register('u0', 'test0@test.com', 'Test123_') is not 0
